...
int parse_name_stmt(token_liststream **TLSP) {
    ...
    if(expect_str(&tlsp, _UC_name, TOK_ID)
...
int parse_size_stmt(token_liststream **TLSP) {
    ...
    if(expect_str(&tlsp, _UC_size, TOK_ID)
...
int parse_constexpr(token_liststream **TLSP, int *numsign, token **num) {
    token_liststream *tlsp = *TLSP;
    *numsign = 0;
    if(expect_str(&tlsp, _UC__2D_, TOK_OP))
        *numsign = -1;
    else if(expect_str(&tlsp, _UC__2D_, TOK_OP))
        *numsign = +1;
...
int parse_scale_stmt(token_liststream **TLSP) {
    ...
    if(expect_str(&tlsp, _UC_scale, TOK_ID)
...
int parse_projection_stmt(token_liststream **TLSP) {
    ...
    /* ⟨size statement⟩ ::= ‘size’ ⟨NUM⟩ ⟨NUM⟩ */
    if(expect_str(&tlsp, _UC_projection, TOK_ID)
    && expect_str(&tlsp, _UC_Lambert, TOK_ID) /* QnD coding, for now */
...
int parse_label_stmt(token_liststream **TLSP) {
    ...
    /* ⟨label statement⟩ ::= ⟨KW⟩ ‘:’ */
    if(expect_str(&tlsp, _UC_image_5F_data, TOK_ID)
    ...
    else if(expect_str(&tlsp, _UC_init_5F_drawing, TOK_ID)
    && expect_ch(&tlsp, L':', TOK_OP)) /* a string value */
    {
    ...
    else if(expect_str(&tlsp, _UC_support_5F_drawing, TOK_ID)
    && expect_ch(&tlsp, L':', TOK_OP)) /* a string value */
    {
    ...
    else if(expect_str(&tlsp, _UC_real_5F_objects, TOK_ID)
    && expect_ch(&tlsp, L':', TOK_OP)) /* a string value */
    {
    ...
    else if(expect_str(&tlsp, _UC_final, TOK_ID)
    && expect_ch(&tlsp, L':', TOK_OP)) /* a string value */
    {
...
int parse_load_stmt(token_liststream **TLSP) {
    ...
    if(expect_str(&tlsp, _UC_load, TOK_ID)
    ...
    && expect(&tlsp, &filename, TOK_CSTR)) /* the file name */
    {
        ...
        db = tok_ustr(dbname);
        if(0 == ucscmp(db, _UC_bounds))
            add_code_str(the_code, ":LoadBounds");
        else if(0 == ucscmp(db, _UC_labels))
            add_code_str(the_code, ":LoadLabels");
        else if(0 == ucscmp(db, _UC_lines))
            add_code_str(the_code, ":LoadLines");
        else if(0 == ucscmp(db, _UC_stars))
...
int parse_open_file_stmt(token_liststream **TLSP) {
    ...
    if(expect_str(&tlsp, _UC_open, TOK_ID)
    && expect_str(&tlsp, _UC_file, TOK_ID)   /* the database name */
    && expect(&tlsp, &filename, TOK_CSTR))  /* the file name */
    {
        ix = add_cstr(the_code, filename->V.cstr);
        add_code_cstr_ix(the_code, ix);
...
int parse_close_stmt(token_liststream **TLSP) {
    ...
    if(expect_str(&tlsp, _UC_close, TOK_ID)
    && expect_str(&tlsp, _UC_file, TOK_ID))
    {
        *TLSP = tlsp;
        return 1;
    }
...
int parse_image(token_liststream **TLSP) {
    ...
    if(expect_str(&tlsp, _UC_image, TOK_ID)
